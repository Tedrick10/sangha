...
<div class="admin-form-page-header">
    ...
    <h1 class="admin-page-title">{{ t('add_role') }}</h1>
</div>

<form action="{{ route('admin.roles.store') }}" method="POST" class="admin-form-card">
    @csrf
    <div class="space-y-6">
        <div class="admin-form-group">
            <label for="name" class="admin-form-label">{{ t('role_name') }} *</label>
            <input type="text" name="name" id="name" value="{{ old('name') }}" required class="admin-input" placeholder="e.g. Admin, Editor">
                                ...
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30">
                                    ...
                                    @foreach(['create','read','update','delete'] as $action)
                                        <td class="px-4 py-2 text-center">
                                            <input type="checkbox" name="permissions[]" value="{{ $resource }}.{{ $action }}" {{ in_array($resource.'.'.$action, old('permissions', [])) ? 'checked' : '' }} class="admin-checkbox">
                                        </td>
                                    @endforeach
                                </tr>
                            @endforeach
    ...
    <div class="admin-form-actions mt-6">
        <button type="submit" class="admin-btn-primary">{{ t('create_role') }}</button>
