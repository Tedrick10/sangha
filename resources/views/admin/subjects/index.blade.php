...
<div class="admin-page-header">
    <h1>Subjects</h1>
    <a href="{{ route('admin.subjects.create') }}" class="admin-btn-add">@include('partials.icon', ['name' => 'plus', 'class' => 'w-5 h-5']) Add Subject</a>
</div>
...
<form method="GET" class="admin-filter-bar flex flex-wrap gap-4 items-end">
    ...
    <div class="flex gap-2">
        ...
        @if(request()->hasAny(['search', 'is_active']))
            <a href="{{ route('admin.subjects.index') }}" class="admin-btn-clear">@include('partials.icon', ['name' => 'x', 'class' => 'w-4 h-4']) Clear</a>
        @endif
    </div>
</form>
...
<div class="admin-table-card overflow-x-auto">
    <div class="flex justify-start px-4 sm:px-6 py-2 border-b border-slate-100 dark:border-slate-600">
        @include('admin.partials.column-visibility', [
            ...
            'storageKey' => 'admin-subjects-columns',
            'columns' => [
                ['id' => 'name', 'label' => 'Name'],
    ...
    <table id="subjects-table" class="admin-table divide-y divide-slate-100">
        <thead>
            <tr>
                ...
                <th data-column="marks">Pass / Full Mark</th>
        ...
        <tbody>
            @forelse($subjects as $subject)
                <tr>
                    ...
                    <td class="text-right">
                        <a href="{{ route('admin.subjects.edit', $subject) }}" class="admin-action-link admin-action-edit">@include('partials.icon', ['name' => 'pencil', 'class' => 'w-4 h-4']) Edit</a>
                        <form action="{{ route('admin.subjects.destroy', $subject) }}" method="POST" class="inline-block ml-2" onsubmit="return confirm('Delete this subject?');">
                            @csrf
                            @method('DELETE')
            ...
            @empty
                <tr>
                    <td colspan="6" class="admin-table-empty">No subjects yet. <a href="{{ route('admin.subjects.create') }}">Create one</a>.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
...
@include('admin.partials.table-pagination', ['paginator' => $subjects, 'routeName' => 'admin.subjects.index'])
@endsection
