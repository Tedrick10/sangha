@php
    ...
    $canMonasteriesRead = $can('monasteries.read');
    $canSanghasRead = $can('sanghas.read');
    $canMonasteryRequestsRead = $can('monastery_requests.read');
    $canMandatoryScoresRead = $can('mandatory_scores.read');
    $canScoreModerationRead = $can('score_moderation.read');
    $canCleanPassRead = $can('clean_pass.read');
    $canSubjectsRead = $can('subjects.read');
    $canExamTypesRead = $can('exam_types.read');
    $canExamsRead = $can('exams.read');
    $canWebsitesRead = $can('websites.read');
    $canCustomFieldsRead = $can('custom_fields.read');
    $canSiteImagesRead = $can('site_images.read');
    $canAppearanceRead = $can('appearance.read');
    $canLanguagesRead = $can('languages.read');
    $canRolesRead = $can('roles.read');
    $canUsersRead = $can('users.read');
    $showExaminationsGroup = $canSubjectsRead || $canExamTypesRead || $canExamsRead;
    $showContentGroup = $canWebsitesRead || $canCustomFieldsRead || $canSiteImagesRead || $canAppearanceRead;
    $showAdministrationGroup = $canLanguagesRead || $canRolesRead || $canUsersRead;
    $showScoreEntryGroup = $canMandatoryScoresRead || $canScoreModerationRead || $canCleanPassRead;

    $isMonasteries = request()->routeIs('admin.monasteries.*');
    $isSanghas = request()->routeIs('admin.sanghas.*');
    $isMandatoryScores = request()->routeIs('admin.mandatory-scores.*', 'admin.clean-pass.*', 'admin.score-moderation.*');
    $isExaminations = request()->routeIs('admin.subjects.*', 'admin.exam-types.*', 'admin.exams.*');
    ...
    if (! in_array($requestFormScope, ['general'], true)) {
...
<nav class="admin-sidebar-nav flex flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden p-2 pb-4">
    ...
    <div class="sidebar-group">
        ...
        <div class="sidebar-group-content {{ $isSanghas ? 'is-expanded' : '' }}">
            ...
            <div class="pl-4 pt-1 pb-2 space-y-0.5">
                ...
                <a href="{{ route('admin.sanghas.index', ['moderation_status' => 'eligible']) }}" class="block px-3 py-2 rounded-lg text-sm transition-colors duration-200 {{ $isSanghas && $sanghaStatus === 'eligible' ? 'bg-amber-500/15 text-amber-900 ring-1 ring-amber-500/25 dark:bg-amber-500/10 dark:text-amber-200 dark:ring-amber-400/30' : 'text-stone-600 hover:bg-stone-100 hover:text-stone-900 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-amber-200' }}">{{ t('mp_card_eligible', 'Eligible') }}</a>
                ...
                <a href="{{ route('admin.sanghas.index', ['moderation_status' => 'needed_update']) }}" class="block px-3 py-2 rounded-lg text-sm transition-colors duration-200 {{ $isSanghas && $sanghaStatus === 'needed_update' ? 'bg-amber-500/15 text-amber-900 ring-1 ring-amber-500/25 dark:bg-amber-500/10 dark:text-amber-200 dark:ring-amber-400/30' : 'text-stone-600 hover:bg-stone-100 hover:text-stone-900 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-amber-200' }}">{{ t('status_needed_update', 'Needed Update') }}</a>
    ...
    {{-- Score Entry → Exam Mark Entry --}}
    @if($showScoreEntryGroup)
    <div class="sidebar-group">
        ...
        <div class="sidebar-group-content {{ $isMandatoryScores ? 'is-expanded' : '' }}">
            ...
            <div class="pl-4 pt-1 pb-2 space-y-0.5">
                ...
                <a href="{{ route('admin.mandatory-scores.grid') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-colors duration-200 {{ request()->routeIs('admin.mandatory-scores.grid') || request()->routeIs('admin.mandatory-scores.grid-row') ? 'bg-amber-500/15 text-amber-900 ring-1 ring-amber-500/25 dark:bg-amber-500/10 dark:text-amber-200 dark:ring-amber-400/30' : 'text-stone-600 hover:bg-stone-100 hover:text-stone-900 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-amber-200' }}">
                    ...
                    {{ t('mandatory_scores_grid_title', 'Exam Mark Entry') }}
                </a>
                @endif
                @if($canScoreModerationRead)
                <a href="{{ route('admin.score-moderation.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-colors duration-200 {{ request()->routeIs('admin.score-moderation.*') ? 'bg-amber-500/15 text-amber-900 ring-1 ring-amber-500/25 dark:bg-amber-500/10 dark:text-amber-200 dark:ring-amber-400/30' : 'text-stone-600 hover:bg-stone-100 hover:text-stone-900 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-amber-200' }}">
                    @include('partials.icon', ['name' => 'eye', 'class' => 'w-4 h-4 shrink-0 text-amber-600/90 dark:text-amber-400/90'])
                    {{ t('menu_score_moderation', 'Moderation') }}
                </a>
                @endif
                @if($canCleanPassRead)
                <a href="{{ route('admin.clean-pass.index') }}" class="flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-colors duration-200 {{ request()->routeIs('admin.clean-pass.*') ? 'bg-amber-500/15 text-amber-900 ring-1 ring-amber-500/25 dark:bg-amber-500/10 dark:text-amber-200 dark:ring-amber-400/30' : 'text-stone-600 hover:bg-stone-100 hover:text-stone-900 dark:text-slate-400 dark:hover:bg-white/10 dark:hover:text-amber-200' }}">
                    <svg class="w-4 h-4 shrink-0 text-amber-600/90 dark:text-amber-400/90" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    {{ t('menu_clean_pass', 'Clean Pass') }}
                </a>
                @endif
            </div>
            </div>
