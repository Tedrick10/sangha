    ...
    <div>
        ...
        <div class="rounded-xl border border-stone-200/90 dark:border-slate-700 bg-stone-50/80 dark:bg-slate-900/50 divide-y divide-stone-200/80 dark:divide-slate-700 overflow-hidden">
            @foreach($languages as $lang)
                ...
                @endphp
                <button type="button" data-locale="{{ $lang->code }}" class="js-app-locale-choice flex w-full items-center justify-between gap-3 px-4 py-3.5 text-left text-sm font-medium transition-colors {{ $isActive ? 'bg-amber-50 dark:bg-amber-900/25 text-amber-900 dark:text-amber-300' : 'text-stone-700 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-800' }}">
                        <span class="flex items-center gap-3 min-w-0">
                            <span class="text-lg leading-none shrink-0">{{ $langFlag }}</span>
                            <span class="truncate">{{ $lang->name }}</span>
                        </span>
                        @if($isActive)
                            <svg class="w-5 h-5 shrink-0 text-amber-600 dark:text-amber-400" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                        @endif
                </button>
            @endforeach
        </div>
    ...
    <div>
        ...
        <div class="rounded-xl border border-stone-200/90 dark:border-slate-700 bg-white dark:bg-slate-900 py-1 shadow-sm overflow-hidden divide-y divide-stone-100 dark:divide-slate-800">
            @foreach(['light', 'dark', 'system'] as $opt)
                <button type="button" data-website-theme="{{ $opt }}" class="website-theme-choice flex w-full min-h-[52px] items-center justify-between gap-2 px-4 py-3 text-left text-sm text-stone-700 dark:text-slate-200 hover:bg-stone-50 dark:hover:bg-slate-800/90 transition-colors {{ $theme === $opt ? 'bg-amber-50/90 dark:bg-amber-900/25 text-amber-800 dark:text-amber-300' : '' }}">
                    ...
                    <svg class="website-theme-check h-5 w-5 shrink-0 text-amber-600 dark:text-amber-400 {{ $theme === $opt ? '' : 'hidden' }}" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" /></svg>
                </button>
            @endforeach
        </div>
    </div>
